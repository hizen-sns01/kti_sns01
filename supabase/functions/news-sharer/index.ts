...
async function summarizeAndPrompt(articleTitle: string, articleUrl: string, context: string): Promise<string> {
  ...
  const prompt = `아래 기사 내용을 바탕으로, 짧고 흥미로운 요약(2-3문장)을 한국어로 작성해 주세요. 요약 시 중요한 키워드는 **굵게** 표시하고, 내용에 따라 적절하게 문단을 나누어 주세요. 이어서, 토론을 시작할 수 있는 생각을 자극하는 질문을 한국어로 제시해 주세요. 이 토론은 "${context}"에 대한 채팅방을 위한 것입니다.\n\n---\n\n기사 내용:\n${truncatedText}`;
